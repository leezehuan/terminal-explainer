...
# [ 步骤 2: 安装 Python 库 (已修复) ]
step "步骤 2/5: 安装 Python 'requests' 库..."
# 优先尝试使用 apt (适用于 Debian/Ubuntu 等)
if command -v apt-get >/dev/null 2>&1; then
    echo "  -> 检测到 apt, 尝试使用 'apt install python3-requests'..."
    if apt-get install -y python3-requests >/dev/null 2>&1; then
        echo "  -> 'python3-requests' 已通过 apt 成功安装。"
    else
        echo "  -> apt 安装失败, 回退到 pip..."
        pip3 install requests --quiet --break-system-packages || {
            echo "  -> 错误: pip 安装也失败了。请检查您的网络或 Python 环境。"
            exit 1
        }
        echo "  -> 警告: 已使用 pip --break-system-packages 强制安装。"
    fi
else
    # 对于没有 apt 的系统 (如 CentOS, Fedora, macOS等)
    echo "  -> 未检测到 apt, 使用 pip 进行安装..."
    pip3 install requests --quiet || {
        echo "  -> pip 安装失败, 尝试使用 '--break-system-packages'..."
        pip3 install requests --quiet --break-system-packages || {
            echo "  -> 错误: pip 安装彻底失败。请手动为 python3 安装 'requests' 库。"
            exit 1
        }
        echo "  -> 警告: 已使用 pip --break-system-packages 强制安装。"
    }
